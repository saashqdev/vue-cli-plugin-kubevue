...
<div v-show="rectStyle" :class="$style.root" :mode="mode" :tag="info.tag" :style="rectStyle"
    ...
    <div :class="$style.bar">
        ...
        <span :class="$style.icon" title="Create a Copy" role="duplicate" @click="duplicate"></span>
        <span :class="$style.icon" title="Remove" role="remove" @click="remove"></span>
        <span v-if="nodeAPI && nodeAPI.control" :class="$style.icon" title="Control" role="control" @click="control"></span>
        <span :class="$style.icon" title="Edit Modal Box" role="modal" @click="edit(item)" v-for="(item, index) in ctrlList" :key=" index"></span>
    </div>
</div>
</template>
    ...
    methods: {
        ...
        remove() {
            ...
            if (nodeInfo.el === this.$parent.contextVM.$el) {
                this.$parent.sendCommand('toast', 'The page root node cannot be deleted!');
                return;
            }
...
.icon[role="modal"]::before {
    ...
    icon-font: url('../assets/popup_window.svg');
}
