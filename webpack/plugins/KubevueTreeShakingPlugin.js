...
class KubevueTreeShakingPlugin {
    constructor(options) {
    ...
    analyzeUnDeadLibModules(libModulesMaps, compilation) {
        ...
        moduleInfo.forEach((info, i) => {
            info.implictTags.length && info.implictTags.forEach((tag) => !tagNames.includes(tag) && compilation.errors.push(new Error(`kubevue build: you should import ${tag} as well`)));
        });
    }
}

module.exports = KubevueTreeShakingPlugin;
